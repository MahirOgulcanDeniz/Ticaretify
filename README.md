...
*   **Secure Authentication & Session Management:** Built-in registration, login, and secure logout flows utilizing native **PHP Sessions**. Passwords are secured using **MD5 encryption**, backed by data validation for character limits and password matching.
...
*   **Personalized Wishlist (Favorites List):** Logged-in users can dynamically curate an interactive wishlist. Features optimized SQL **JOIN** queries to securely cross-reference user records with product attributes.
